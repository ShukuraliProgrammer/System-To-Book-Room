...
add_subcategory = '➕ Sub Kategoriyani qushish'
...
async def process_settings(message: Message):
    ...
    markup.add(InlineKeyboardButton(
        '+ Kategoriya qoshish', callback_data='add_category'))

    await message.answer('katigoriyani sozlash:', reply_markup=markup)


@db.callback_query_handler(IsAdmin(), category_cb.filter(action='view'))
async def category_callback_handler(query: CallbackQuery, callback_data: dict, state: FSMContext):
    category_idx = callback_data['id']
    subcategories = db.fetchall('''
        SELECT * FROM subcategories subcategory
    WHERE subcategory.category = ?)''',
                           (category_idx,))

    await query.message.delete()
    await query.answer("Ushbu kategoriyalarga qushilgan Sub Kategoriyalar")
...
async def category_callback_handler(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    products = db.fetchall('''SELECT * FROM subcategories subcategory
    WHERE subcategory.tag = (SELECT title FROM categories WHERE idx=?)''',
                           (category_idx,))
...
async def process_confirm(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        print("dateL: ", data["category_index"])
