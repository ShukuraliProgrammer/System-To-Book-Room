...
async def process_questions(message: Message):
    ...
    if len(questions) == 0:
        ...
        await message.answer('Savol yoq.')
...
async def process_submit(message: Message, state: FSMContext):
    ...
    await message.answer('Javobingizni xato yozmasligingizga ishonch hosil qiling.', reply_markup=submit_markup())
...
async def process_send_answer(message: Message, state: FSMContext):
    await message.answer('Bekor qilish!', reply_markup=ReplyKeyboardRemove())
    await state.finish()
...
async def process_send_answer(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.answer('Yuborish!', reply_markup=ReplyKeyboardRemove())
