...
async def cmd_sos(message: Message):
    ...
    await message.answer('Muammoning mohiyati nimada? Iloji boricha batafsil tavsiflang va administrator sizga javob beradi.', reply_markup=ReplyKeyboardRemove())
...
async def process_question(message: Message, state: FSMContext):
    ...
    await message.answer('Hamma narsa tugri ekanligiga ishonch hosil qiling.', reply_markup=submit_markup())
    await SosState.next()
...
async def process_cancel(message: Message, state: FSMContext):
    await message.answer('Bekor qilmoq!', reply_markup=ReplyKeyboardRemove())
    await state.finish()
